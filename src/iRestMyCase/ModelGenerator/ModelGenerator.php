...
class ModelGenerator {
	...
	static $_numericTypes = ['bigint', 'decimal', 'double', 'float', 'int', 'integer', 'mediumint', 'numeric', 'smallint', 'tinyint'];
	...
	function __construct($tableName = NULL, $classDbFields = NULL){
		...
		if(isset($classDbFields)){ $this->classDbFields($classDbFields); }
	}

	public function className($value = NULL)
	{
		if(!empty($value)){
			if(strpos($value, 'v_') === 0 ){$value = substr($value, 2); }
	...
	{
		if(!empty($classDbFields) && is_array($classDbFields)){ $this->classDbFields = $value; }
		return $this->classDbFields;
	}

	public function extendedClassName($value = NULL)
	{
		if(!empty($value)){ $this->extendedClassName = $value; }
...
";
	...
	public function classProperties($tableColumns)
	{
		...
		foreach($tableColumns as &$column){
			...
			elseif(in_array($column['ShortType'], self::$_stringTypes) && $column['Null'] == 'NO' && empty($column['Key'])){
				$propertyValue = " = ''";
			}
			$propertiesContent .= "
...
";
		...
		elseif(\$value === false){
			...
			elseif(in_array($column['ShortType'], self::$_stringTypes)){
				foreach($tableColumns as $tmpColumn){ $columnNames[] = $tmpColumn['Field'];}
				if(substr($column['Field'],-3) == '_en' && in_array(substr($column['Field'],0,-3). '_fr', $columnNames)){
